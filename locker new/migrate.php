...
// 7. Ensure guide_approval column exists
$check = $conn->query("SHOW COLUMNS FROM allocations LIKE 'guide_approval'");
if ($check->num_rows === 0) {
    $conn->query("ALTER TABLE allocations ADD COLUMN guide_approval ENUM('pending','approved','rejected') DEFAULT 'approved'");
    $results[] = "✅ Added guide_approval to allocations";
} else $results[] = "⏭️ guide_approval already exists";
