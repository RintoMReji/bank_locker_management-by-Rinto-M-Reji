...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        ...
        if ($req) {
            ...
            $conn->commit();
            $msg = "Delete request approved. Locker surrendered.";
        }
        ...
        $msg = "Delete request rejected.";
    }
}

$requests = $conn->query("
    SELECT dr.*, c.full_name, c.customer_id AS cid, a.allocation_no, l.locker_number, l.locker_size
    FROM delete_requests dr JOIN customers c ON dr.customer_id=c.id
    JOIN allocations a ON dr.allocation_id=a.id JOIN lockers l ON a.locker_id=l.id
...
<div class="card">
  <div class="card-header"><h3>🗑️ Locker Delete Requests</h3></div>
  <div class="card-body">
    ...
    <div class="approval-card">
      ...
      <form method="POST" class="approval-actions">
        ...
        <div class="form-group"><input type="text" name="remarks" class="form-control" placeholder="Remarks"></div>
        <button type="submit" name="action" value="approve" class="btn btn-success" onclick="return confirm('Approve deletion?')">✅ Approve</button>
        <button type="submit" name="action" value="reject" class="btn btn-danger">❌ Reject</button>
      </form>
      <?php endif; ?>
    </div>
    <?php endwhile; ?>
    <?php if(!$found): ?><div class="text-center" style="padding:40px;color:#888;">No delete requests.</div><?php endif; ?>
  </div>
</div>
