...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($chk['status'] !== 'available') {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO allocations (allocation_no,customer_id,locker_id,allocation_date,expiry_date,rent_paid,payment_status,remarks) VALUES (?,?,?,?,?,?,?,?)");
            $stmt->bind_param("siissdss", $alloc_no, $cust_id, $lock_id, $alloc_dt, $expiry, $rent, $pay_st, $remarks);
            $stmt->execute();
